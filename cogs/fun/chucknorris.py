...
class chucknorris(commands.Cog):
    ...
    async def on_message(self, message):
        if message.author.id in data["whitelisted"]:
            if message.content == f"{data['prefix']}chucknorris":
                await message.reply(f"**spooky.wtf - Discord Self-Bot**\n\n> **[!]** {requests.get('https://api.chucknorris.io/jokes/random').json()['value']}\n\n-# Created by @ s.eths・v{data['version']}")
                os.system("cls"), print(logo()), print(login_box(self.bot.user)), print(output_text(f"[<>] `{data['prefix']}chucknorris` has been successfully ran by {message.author}"))
