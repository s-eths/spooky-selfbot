...
class penis(commands.Cog):
    ...
    async def on_message(self, message):
        if message.author.id in data["whitelisted"]:
            if message.content.startswith(f"{data['prefix']}penis"):
                ...
                await message.reply(f"**spooky.wtf - Discord Self-Bot**\n\n> **[!]** {user.mention}'s penis 8{length}Đ | {len(length)} inches 🍆\n\n-# Created by @ s.eths・v{data['version']}")
                os.system("cls"), print(logo()), print(login_box(self.bot.user)), print(output_text(f"[<>] `{data['prefix']}penis @{user.name}` has been successfully ran by {message.author}"))
